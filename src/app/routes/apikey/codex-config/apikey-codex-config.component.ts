...
import { CodexConfigPayload, CodexConfigPreview, PlatformKey } from '../apikey.model';
import { PlatformKeysService } from '../apikey.service';

@Component({
  selector: 'app-platform-key-codex-config',
  templateUrl: './apikey-codex-config.component.html',
  styleUrls: ['./apikey-codex-config.component.less'],
...
export class PlatformKeyCodexConfigComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    forkJoin({
      ...
      .subscribe(({ keys, metrics }) => {
        ...
        this.platformKeys = keys.filter((item) => item.enabled);
  ...
  protected get maskedKey(): string {
    const prefix = this.selectedKey?.keyPrefix || '';
    return prefix ? `${prefix}${'•'.repeat(18)}` : '-';
  }
